...
select IdCategoria,COUNT(*) AS cantidad FROM productos GROUP BY IdCategoria;
/* ¿Cuál es el precio de compra más alto y más bajo entre todos los productos?*/
select max(precioCompra) as maximo,min(precioCompra) as minimo from productos;
...
datos?*/
select vendedor from productos;
/*¿Cuál es la cantidad total de productos en existencia para cada vendedor?*/
select vendedor, sum(cantidad) as cantidad FROM productos GROUP BY vendedor;
/*¿Cuál es el precio de venta sugerido para cada producto en función de la
escala?*/
select nombre,escala,precioventasugerido from productos;
...
promedio?*/
SELECT IdCategoria,AVG(precioventasugerido) AS precio_sugerido FROM  productos GROUP BY  IdCategoria;
/*¿Cuál es el nombre y la cantidad de productos vendidos por un vendedor en
particular?*/
select nombre,cantidad from productos where vendedor = "Min Lin Diecast";



/*________________________________________________________________________________________________*/
show tables;
/* De la tabla Clientes realizar las siguientes consultas:*/
...
específica?*/
select NombreContacto,ApellidoContacto from clientes where Ciudad ="San Rafael";
/*¿Cuál es la lista de todos los empleados asociados a cada cliente? */
select empleados.Nombre, clientes.NombreCliente from clientes join empleados on empleados.IdEmpleado= clientes.IdEmpleado;
/*¿Cuál es la cantidad de clientes en cada estado? */
select count(nombreCliente),estado from clientes group by Estado;
...
select nombreCliente,Direccion1,Direccion2,ciudad,estado,codigoPostal,pais from clientes;
/*¿Cuál es el promedio de límite de crédito para todos los clientes en una
ciudad específica? */
SELECT AVG(limite_credito) AS promedio_limite_credito FROM clientes WHERE ciudad = 'ciudad_especifica';

/*  ¿Cuál es el cliente con el límite de crédito más alto y más bajo?*/
SELECT nombre_empresa, limite_credito FROM clientes ORDER BY limite_credito DESC LIMIT 1;
SELECT nombre_empresa, limite_credito FROM clientes ORDER BY limite_credito ASC LIMIT 1;

/* ¿Cuál es la lista de clientes cuyo nombre de contacto contiene un término
específico?*/
SELECT nombre_empresa, nombre_contacto FROM clientes WHERE nombre_contacto LIKE '%Atelier graphique%';
...
/*  
1. ¿Cuáles son todas las ciudades donde se encuentran las oficinas registradas?*/
SELECT DISTINCT ciudad FROM oficinas;
/* ¿Cuál es el número de teléfono de cada oficina en la base de datos?*/
SELECT telefono FROM oficinas;
/* ¿Cuál es la dirección completa de cada oficina, incluyendo dirección 1, dirección 2, ciudad, estado, código postal y país?*/
SELECT direccion1, direccion2, ciudad, estado, codigopostal, pais FROM oficinas;
/* ¿Cuál es el territorio asignado a cada oficina? */
SELECT territorio FROM oficinas;
/*  ¿Cuál es el número de oficinas en cada país?*/
SELECT pais, COUNT(*) AS numero_oficinas FROM oficinas GROUP BY pais;
/*¿Cuál es el código postal de cada oficina?  */
SELECT codigopostal FROM oficinas;
/*  ¿Cuál es la lista de todas las oficinas en un estado específico?*/
SELECT * FROM oficinas WHERE estado = 'California';
/*   ¿Cuál es el promedio de teléfonos por oficina en una ciudad específica?*/
SELECT ciudad, AVG(numero_telefonos) AS promedio_telefonos FROM oficinas GROUP BY ciudad;
/* ¿Cuál es la lista de todas las oficinas con un territorio particular?*/
SELECT * FROM oficinas WHERE territorio = 'América del Norte';
/*  ¿Cuál es la lista de todas las oficinas cuya dirección contiene un término específico?*/
SELECT * FROM oficinas WHERE direccion1 LIKE '%Calle 123%';
